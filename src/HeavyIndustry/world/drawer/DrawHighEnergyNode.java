...
public class DrawHighEnergyNode extends DrawBlock {
    public TextureRegion bridgeRegion;
    public TextureRegion endRegion;

    @Override
    public void draw(Building build) {
        if(!(build instanceof HighEnergyNodeBuild)) return;
        drawR((HighEnergyNodeBuild) build);
    }
    public void drawR(HighEnergyNodeBuild build){
        Draw.z(Layer.power);
