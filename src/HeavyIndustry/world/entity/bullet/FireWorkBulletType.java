package HeavyIndustry.world.entity.bullet;

import HeavyIndustry.content.HIGet;
import arc.Core;
...
public class FireWorkBulletType extends BulletType {
    ...
    public void drawTrail(Bullet b) {
        if(trailLength > 0 && b.trail != null){
            float z = Draw.z();
            Draw.z(z - 0.0001f);
            b.trail.draw(colorful ? HIGet.EC1.set(HIGet.rainBowRed).a(0.7f).shiftHue(b.time * 2) : color, trailWidth);
            Draw.z(z);
        }
    }
    ...
    public void draw(Bullet b) {
        super.draw(b);
        if(outline){
            Draw.color(colorful ? HIGet.EC2.set(HIGet.rainBowRed).shiftHue(b.time * 2) : color);
    ...
    public static class colorFire extends BulletType{
        ...
        public void draw(Bullet b) {
            ...
            Draw.color(b.data == Color.white ? HIGet.EC18.set(HIGet.rainBowRed).shiftHue(b.time * 2) : (Color)b.data);
            Draw.z(Layer.bullet);
            for(int i = 0; i < 4; i++) {
        ...
        public void drawTrail(Bullet b) {
            if(trailLength > 0 && b.trail != null){
                float z = Draw.z();
                Draw.z(z - 0.0001f);
                b.trail.draw(b.data == Color.white ? HIGet.EC19.set(HIGet.rainBowRed).shiftHue(b.time * 2) : (Color)b.data, trailWidth);
                Draw.z(z);
            }
        }
    }
    public static class spriteBullet extends BulletType{
        ...
        public void draw(Bullet b) {
            ...
            if(!(b.data instanceof Color)) return;
            Draw.z(Layer.bullet);
            Draw.color(b.data == Color.white ? HIGet.EC20.set(HIGet.rainBowRed).shiftHue(b.time * 2) : (Color)b.data);
