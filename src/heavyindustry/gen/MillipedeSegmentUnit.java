...
public class MillipedeSegmentUnit extends UnitEntity {
    ...
    @Override
    public int classId() {
        return EntityRegister.getId(MillipedeSegmentUnit.class);
    }
    ...
    public void updateSegment() {
        if (trueParentUnit != null) {
            if (millipedeType.splittable && millipedeType.healthDistribution <= 0f)
    ...
    protected void split() {
        ...
        if (index < 1 || index >= hd.segmentUnits.length - 1) {
            return;
        }
    ...
    protected void updateWeapon() {
        ...
        for (WeaponMount mount : mounts) {
            ...
            float mountX = x + Angles.trnsx(rotation - 90, weapon.x, weapon.y);
            float mountY = y + Angles.trnsy(rotation - 90, weapon.x, weapon.y);
    ...
    protected void shoot(WeaponMount mount, float x, float y, float aimX, float aimY, float mountX,
        ...
        if (delay) {
            Utils.shotgun(weapon.shoot.shots, weapon.reload, rotation, (f) -> {
            ...
            Utils.shotgun(weapon.shoot.shots, weapon.reload, rotation, f -> mount.bullet = bullet(weapon, x, y, f + Mathf.range(weapon.inaccuracy), lifeScl));
        }
    ...
    protected Bullet bullet(Weapon weapon, float x, float y, float angle, float lifescl) {
        return weapon.bullet.create(this, team, x, y, angle, 1.0f - weapon.velocityRnd + Mathf.random(weapon.velocityRnd), lifescl);
    }
    ...
    protected static class SegmentData {
        protected MillipedeSegmentUnit[] units;
        protected Vec2[] pos;
        protected Vec2[] vel;
        protected int size = 0;

        protected SegmentData(int size) {
            units = new MillipedeSegmentUnit[size];
            pos = new Vec2[size];
            vel = new Vec2[size];
        }

        protected void add(MillipedeSegmentUnit unit, Vec2 vel) {
            units[size] = unit;
            pos[size] = new Vec2(unit.getX(), unit.getY());
            this.vel[size++] = vel;
        }

        protected void add(MillipedeDefaultUnit unit, int index) {
            Log.info(this + ":" + unit.segmentUnits[index] + ":" + unit.segments[index] + ":" + unit.segmentVelocities[index] + ":" + index);
            units[size] = unit.segmentUnits[index];
            pos[size] = unit.segments[index];
            vel[size++] = unit.segmentVelocities[index];
        }

        protected void set(MillipedeDefaultUnit unit) {
