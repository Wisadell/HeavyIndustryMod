...
public class LegsPayloadLegacyUnit extends Unit implements Boundedc, Builderc, Drawc, Entityc, Flyingc, Healthc, Hitboxc, Itemsc, Legsc, Minerc, Payloadc, Physicsc, Posc, Rotc, Shieldc, Statusc, Syncc, Teamc, Unitc, Velc, Weaponsc {
    public static final float hitDuration = 9.0F;

    protected static final Vec2 straightVec = new Vec2();
    ...
    public transient Vec2 curMoveOffset = new Vec2();
    ...
    @Nullable
    protected transient PowerGraph payloadPower;
    ...
    protected LegsPayloadLegacyUnit() {
    }

    @Override
    public Vec2 curMoveOffset() {
        return curMoveOffset;
    }
    ...
    @Override
    public int classId() {
        return EntityRegistry.getID(LegsPayloadLegacyUnit.class);
    }
    ...
    @Override
    public void curMoveOffset(Vec2 curMoveOffset) {
    ...
    @Override
    public void lastDeepFloor(Floor lastDeepFloor) {
    ...
    public void read(Reads read) {
        ...
        if(REV == 0) {
            ...
            throw new IllegalArgumentException("Unknown revision '" + REV + "' for entity type 'desolationLord'");
        }
    ...
    public boolean canPickup(mindustry.gen.Unit unit) {
    ...
    public float defaultLegAngle(int index) {
        ...
        return baseRotation + 360.0F / legs.length * index + (360.0F / legs.length / 2.0F);
    }
    ...
    public float legAngle(int index) {
        ...
        if (type.legStraightness > 0) {
            return Mathf.slerp(defaultLegAngle(index), (index >= legs.length / 2 ? -90 : 90.0F) + baseRotation, type.legStraightness);
        }
    ...
    public static LegsPayloadLegacyUnit create() {
        return new LegsPayloadLegacyUnit();
    }
    ...
    public void destroy() {
        legs: {
            ...
            if (!isAdded() || Vars.headless) break legs;
            float legExplodeRad = type.legRegion.height / 4.0F / 1.45F;
            for (int i = 0; i < legs.length; i++) {
                ...
                for (Vec2 vec : new Vec2[]{base, l.joint, l.base}) {
                    Damage.dynamicExplosion(vec.x, vec.y, 0.0F, 0.0F, 0.0F, legExplodeRad, state.rules.damageExplosions, false, team, type.deathExplosionEffect);
                }
                Fx.legDestroy.at(base.x, base.y, 0.0F, new LegDestroyData(base.cpy(), l.joint, type.legRegion));
                Fx.legDestroy.at(l.joint.x, l.joint.y, 0.0F, new LegDestroyData(l.joint.cpy().add(Tmp.v2), l.base, type.legBaseRegion));
            }
        }
        unit: {

            if (!isAdded() || !type.killable) break unit;
    ...
    public void pickup(mindustry.gen.Unit unit) {
        unit.remove();
    ...
    public void update() {
        ...
        legs: {
            ...
            for (int i = 0; i < legs.length; i++) {
                ...
                boolean backLeg = Math.abs((i + 0.5F) - legs.length / 2.0F) <= 0.501F;
