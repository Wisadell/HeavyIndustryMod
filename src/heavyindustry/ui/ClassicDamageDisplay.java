...
public class ClassicDamageDisplay {
    ...
    public static Effect damageShowEffect = new Effect(60f, 80f, (e) -> {
        ...
        Color color = damage == 0 ? Color.gray : data.type == 2 ? Pal.heal : data.target.color;
        ...
        Fonts.def.draw(Strings.fixed(damage, 0), e.x + (forDraw == null ? 0f : forDraw.getRegion().width * realScale / 4f), e.y + (forDraw == null ? 0f : forDraw.getRegion().height * realScale / 4f), Draw.getColor(), realScale, false, Align.left);
        // hello again meep
        if (bullet != null && bullet.owner instanceof Building b && b.block.name.equals("prog-mats-caliber") && damage > bullet.type.damage) {
            ...
            Fonts.def.draw("CRITICAL", e.x, e.y - 16f, critColor, 1f, false, Align.center);
            Fonts.def.draw("HIT!!!", e.x, e.y - 32f, critColor, 1f, false, Align.center);
        }
    ...
    static {
        Events.on(UnitDamageEvent.class, e -> {
            if (!Core.settings.getBool(name("classic-damage-display"))) return;
    ...
    public ClassicDamageDisplay() {
        ...
        Events.run(Trigger.update, () -> {
            if (!Core.settings.getBool(name("classic-damage-display"))) {
    ...
    protected static class DamageInfo {
        int type;
