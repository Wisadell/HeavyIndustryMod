    ...
    private String getData(Building building, PowerInfoType type){
        return switch(type){
            case producer -> {
                ...
                yield bundle.format("hi-power-info-persec", "[#98ffa9]+" + HIUI.formatAmount(produced * 60));
            }
            case consumer -> {
                var consumePower = building.block.consPower;
                float consumed = consumePower.requestedPower(building) * building.timeScale();
                yield bundle.format("hi-power-info-persec", "[#e55454]-" + HIUI.formatAmount(consumed * 60));
            }
            case battery -> {
                var consumePower = building.block.consPower;
                float stored = building.power.status * consumePower.capacity;
                yield "[#fbad67]" + HIUI.formatAmount(stored) + "[gray]/[]" + HIUI.formatAmount(consumePower.capacity);
            }
        };
    }
