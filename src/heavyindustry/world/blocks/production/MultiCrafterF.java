 ...
 */
public class MultiCrafterF extends Block {
    public float warmupSpeed = 0.02f;
    public int[] capacities = {};
    public Seq<Recipe> recipeSeq = new Seq<>();
    public DrawBlock drawer = new DrawDefault();

    public MultiCrafterF(String name) {
        super(name);
        destructible = true;
        envEnabled = Env.any;
        ...
        consume(new ConsumeItemDynamicF((MultiCrafterBuildF tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputItems : null));
        consume(new ConsumeLiquidDynamicF((MultiCrafterBuildF tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputLiquids : null));
        consume(new ConsumePowerDynamic(usage -> usage instanceof MultiCrafterBuildF tile ? tile.getInputPower() : 0f));

        consume(new ConsumeShowStat((MultiCrafterBuildF tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputItems : null, (MultiCrafterBuildF tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputLiquids : null));

        configurable = true;
        saveConfig = true;

        config(Integer.class, (MultiCrafterBuildF tile, Integer index) -> {
        ...
        config(Recipe.class, (MultiCrafterBuildF tile, Recipe val) -> {
            if (!configurable) return;
        ...
        configClear((MultiCrafterBuildF tile) -> tile.currentRecipeIndex = -1);
    }
    ...
    public class MultiCrafterBuildF extends Building {
