...
public class Assembler extends Block {
    ...
    public Assembler(String name) {
        ...
        consume(new ConsumeItemDynamicF((AssemblerBuild tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputItems : null));
        consume(new ConsumeLiquidDynamicF((AssemblerBuild tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputLiquids : null));
        consume(new ConsumePowerDynamic(usage -> usage instanceof AssemblerBuild tile ? tile.getInputPower() : 0f));

        consume(new ConsumeShowStat((AssemblerBuild tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputItems : null, (AssemblerBuild tile) -> tile.currentRecipeIndex != -1 ? recipeSeq.get(Math.min(tile.currentRecipeIndex, recipeSeq.size - 1)).inputLiquids : null));

        configurable = true;
        saveConfig = true;

        config(Integer.class, (AssemblerBuild tile, Integer index) -> {
        ...
        config(Recipe.class, (AssemblerBuild tile, Recipe val) -> {
            if (!configurable) return;
        ...
        configClear((AssemblerBuild tile) -> tile.currentRecipeIndex = -1);
    }
