...
public class ModelPropDrawer implements Disposable {
    ...
    public PropData getData(Mesh mesh) {
        return data.get(mesh, () -> {
            if (!arrayEq(mesh.attributes, attributes, attribEq))
                throw new IllegalArgumentException("Mesh must only have color, 3D position, and normals.");
    ...
    public void render() {
        ...
        for (Req req : requests) {
            ...
            float[] input = data.vertices;
            if (input.length >= vertices.length - vertexOffset * vertStride || data.indices.length >= indices.length - indexOffset)
                flush();
            ...
            for (int i = 0; i < input.length / attribStride; i++) {
                ...
                Vec3 p = Mat3D.prj(vec.set(input[src], input[src + 1], input[src + 2]), trns);
                vertices[dst] = p.x;
                vertices[dst + 1] = p.y;
                vertices[dst + 2] = p.z;

                Vec3 n = Mat3D.prj(vec.set(input[src + 3], input[src + 4], input[src + 5]), nor.set(trns).toNormalMatrix()).nor();
                vertices[dst + 3] = n.x;
                vertices[dst + 4] = n.y;
                vertices[dst + 5] = n.z;
